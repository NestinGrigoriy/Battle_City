...
class PatrolStrategy:
    def __init__(self, x, y, fin_x, fin_y):
        ...
        Атрибуты:
            - self.x, self.y: Текущие координаты танка.
            - self.fin_x, self.fin_y: Целевые координаты для патрулирования.
            - self.angle: Угол поворота танка.
            - self.image: Изображение танка.
            - self.current_axis: Текущая ось движения ("x" или "y").
        """
        self.x = x
        self.y = y
        self.fin_x = fin_x
    ...
    def update(self, screen, speed, hp, armor):
        ...
        pygame.draw.rect(
            screen, "white", (self.x - 35, self.y - 70, armor, 10)
        )
    ...
    def set_new_point(self, x, y):
        ...
        Действия:
            - Обновляет атрибуты self.fin_x и self.fin_y.
             - Сбрасывает текущую ось движения на "x".
        """
        self.fin_x = x
        self.fin_y = y
        self.current_axis = "x"
...
class AttackerStrategy:
    ...
    def __init__(self, x, y, fin_x, fin_y):
        ...
        Атрибуты:
            - self.x, self.y: Текущие координаты танка.
            - self.fin_x, self.fin_y: Целевые координаты.
            - self.angle: Угол поворота танка.
            - self.image: Изображение танка.
            - self.old_x, self.old_y: Предыдущие координаты танка.
            - self.path: Путь, который должен пройти танк.
        """
        self.x = x
        self.y = y
        self.fin_x = fin_x
    ...
    def find_path(self, map_data, start, end, tank_size=75, cell_size=40):
        """
        Находит путь для танка размером tank_size x tank_size в
        лабиринте map_data,
        учитывая, что танк центрируется по середине.

        :param map_data: Список строк, представляющий лабиринт.
        :param start: Кортеж (x, y) — начальная точка (центр танка).
        :param end: Кортеж (x', y') — конечная точка (центр танка).
        ...
        while queue:
            ...
            if (x, y) == end:
                ...
                self.path = path[::-1]
                self.fin_x = self.path[0][0]
                self.fin_y = self.path[0][1]
                del self.path[0]
            ...
            for dx, dy in directions:
                ...
                if 0 <= nx < len(map_data[0]) and 0 <= ny < len(map_data):
                    ...
                    max_cell_x = min(
                        len(map_data[0]) - 1, tank_right // cell_size
                    )
                    min_cell_y = max(0, tank_top // cell_size)
                    max_cell_y = min(
                        len(map_data) - 1, tank_bottom // cell_size
                    )
    ...
    def update(self, screen, speed, hp, armor):
        ...
        if (
            len(self.path) != 0
            and (self.fin_x == self.x)
            and (self.fin_y == self.y)
        ):
            self.fin_x = self.path[0][0]
            self.fin_y = self.path[0][1]
            del self.path[0]
...
class ChaserStrategy:
    ...
    def __init__(self, x, y, map_data):
        ...
        Атрибуты:
            ...
            - self.angle: Угол поворота танка.
            - self.image: Изображение танка.
            - self.grid: Сетка карты, где 1 — непроходимая клетка, 0 — проходимая.
            - self.path: Путь к игроку.
            - self.target_x, self.target_y: Текущая цель танка.
            - self.updatable: Флаг, указывающий, нужно ли обновить путь.
        """
        self.x = x
        self.y = y
        self.angle = 0
    ...
    def create_grid(self, map_data):
        ...
        Действия:
            ...
            - Сохраняет сетку в атрибут self.grid.
        """
        grid = []
        for row in map_data:
            grid_row = []
    ...
    def a_star_search(self, start, goal):
        ...
        :param goal: Целевые координаты (x', y').
        ...
        while open_set:
            ...
            for dx, dy in neighbors:
                ...
                if 0 <= neighbor[0] < len(self.grid) and 0 <= neighbor[
                    1
                ] < len(self.grid[0]):
    ...
    def is_valid_move(self, new_x, new_y):
        ...
        for px, py in points_to_check:
            ...
            grid_x = px // 40
            if not (
                0 <= grid_y < len(self.grid)
                and 0 <= grid_x < len(self.grid[0])
            ):
                return False
    ...
    def can_stand_at(self, i, j):
        ...
        for px, py in corners:
            ...
            grid_j = px // 40
            if not (
                0 <= grid_i < len(self.grid)
                and 0 <= grid_j < len(self.grid[0])
            ):
                return False
            if self.grid[grid_i][grid_j] == 1:
                return False
        return True

    def update(self, screen, speed, hp, armor, player_x, player_y):
        """
        Обновляет положение танка на экране и его состояние.
        :param screen: Экран, на котором отображается танк (pygame.Surface).
        :param speed: Скорость движения танка (float).
        ...
        :param player_y: y - координата игрока(int).
        Действия:
        ...
        start = (self.y // 40, self.x // 40)
        goal = (player_y // 40, player_x // 40)

        if (
            not self.path
            or (self.target_x, self.target_y) == (self.x, self.y)
            or self.updatable
        ):
            ...
            if self.path:
                ...
                self.target_x = next_step[1] * 40
                self.target_y = next_step[0] * 40
        ...
        if self.path:
            ...
            if self.is_valid_move(self.x + move_x, self.y + move_y):
                self.x += move_x
                self.y += move_y
